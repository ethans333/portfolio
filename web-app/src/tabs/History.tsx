import { TextScramble } from "@/components/motion-primitives/hidden-word-text-scramble";
...
export default function History() {
  return (
    <div className="flex flex-col items-center justify-center py-10">
      <CurrentPosition companyName="Enludio" startDate="September 2025" />
      <VDotLine />
      <WorkHistory
...
function WorkCard({ children }: { children: React.ReactNode }) {
  return (
    <Tilt rotationFactor={8} isRevese>
      <div className="w-[90vw] xl:w-[37vw] max-w-[600px] px-7 pt-8 pb-7 border border-zinc-950/10 bg-white dark:border-zinc-50/10 dark:bg-zinc-900 rounded-[12px]">
        {children}
      </div>
    </Tilt>
  );
...
function CurrentPosition({ companyName, startDate }: { companyName: string; startDate: string }) {
  return (
    <WorkCard>
      <div className="lg:flex justify-between items-center">
        <h2 className="mb-1">{companyName}</h2>
      ...
      <p className="py-2">
        <TextScramble className="font-mono text-sm line-clamp-3" duration={Infinity} characterSet=". ">
          ...
          ullamco aliquip laborum est laborum esse cupidatat. Id eiusmod tempor cillum non nostrud fugiat id duis. Sint
          do in ut cillum labore ad magna. Aute pariatur anim duis exercitation ad pariatur cillum esse in labore.
        </TextScramble>
        <br />
        <div className="flex space-x-10">
          <TextScramble className="font-mono text-sm" duration={Infinity} characterSet=". ">
            .xxxxxxxxxxxxxx
          </TextScramble>
          <TextScramble className="font-mono text-sm" duration={Infinity} characterSet=". ">
            .ipsum
